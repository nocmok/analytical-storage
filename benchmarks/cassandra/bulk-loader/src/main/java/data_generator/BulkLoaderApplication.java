...
public class DataGeneratorApplication implements CommandLineRunner {
    ...
    private Integer batchesToInsert = 100000;
    private Integer nThreads = 2000;
    private Random random = new Random();

    public static void main(String[] args) {
        var app = new SpringApplication(DataGeneratorApplication.class);
    ...
    private List<UserEvent> getBatch(int batchSize) {
        ...
        for (int i = 0; i < batchSize; ++i) {
            batch.add(new UserEvent((long) (random.nextInt(100000) + 1),
    ...
    @Override public void run(String... args) throws Exception {
        if(args.length < 1) {
            System.out.println("usage: bulk-loader <rows to insert>");
            System.exit(1);
        }
        batchesToInsert = (Integer.parseInt(args[0]) + batchSize - 1) / batchSize;
        try {
